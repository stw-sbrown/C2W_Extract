create or replace
PROCEDURE P_FIN_TRAN_SERVICE_COMPONENT(no_batch    IN MIG_BATCHSTATUS.NO_BATCH%TYPE,
...
--
-- FILENAME       : P_FIN_TRAN_SERVICE_COMPONENT.sql
--
-- Subversion $Revision: 5362 $
--
-- CREATED        : 01/08/2016
--
-- DESCRIPTION    : Procedure to populate MO_SERVICE_COMPONENT from SAP and OWC supplied data 
--               - OWC_SERVICE_COMPONENT and SAP_SERVICE_COMPONENT.
--
-- NOTES  :
...
-- ---------   ----------  -------    ---------------------------------------------------
-- V 0.01      01/08/2016  S.Badhan   Initial Draft
-----------------------------------------------------------------------------------------

  c_module_name                 CONSTANT VARCHAR2(30) := 'P_FIN_TRAN_SERVICE_COMPONENT';
...
WHEN OTHERS THEN
     ...
     P_MIG_BATCH.FN_UPDATEJOB(no_batch, l_job.NO_INSTANCE, l_job.IND_STATUS);
     RETURN_CODE := -1;
END P_FIN_TRAN_SERVICE_COMPONENT;
/
show errors;
