create or replace
PROCEDURE P_SAP_DEL_SCM_TE(no_batch IN MIG_BATCHSTATUS.NO_BATCH%TYPE,
...
-- AUTHOR         : Kevin Burton
--
-- FILENAME       : P_SAP_DEL_SCM_TE.sql
--
-- Subversion $Revision: 4974 $
--
-- CREATED        : 10/06/2016
--
-- DESCRIPTION    : Procedure to create the SAP SCM upload file
--                  Queries Transform tables and populates table SAP_DEL_SCMTE
--                  Writes to file SAP_DEL_SCMTE_<date/timestamp>.dat
-- NOTES  :
...
-----------------------------------------------------------------------------------------
  ...
  c_module_name                 CONSTANT VARCHAR2(30) := 'P_SAP_DEL_SCM_TE';
...
--  l_delimiter                   VARCHAR2(1) := chr(9); 
  ...
  l_prefix VARCHAR2(6) := 'TE_';

  CURSOR cur_tab IS
    SELECT l_prefix || NVL(SCM.SAPFLOCNUMBER,SCM.STWPROPERTYNUMBER_PK) || '_' || MDP.DPID_PK || '_' || MDP.SERVICECOMPTYPE LEGACYRECNUM,
       PRM.LEGACYRECNUM PARENTLEGACYRECNUM,
       MDP.SERVICECOMPTYPE SERVICECOMPONENTTYPE,
       MDP.DPEFFECTFROMDATE EFFECTIVEFROMDATE,
       MDP.DPID_PK, 
       MDP.NO_IWCS,
--       MDP.SERVICECOMPTYPE TECOMPONENTTYPE, -- V 0.05
       MDP.DPID_TYPE, -- V 0.05
       MDP.NO_SAMPLE_POINT,
       MDP.CONSENT_NO,
       MDP.VOLUME_LIMIT, -- V 0.04 
--       NULL CALD_TXT, -- V 0.03
       SCM.LEGACYRECNUM MAININST,
       MDP.SPID_PK,
       SCM.SAPFLOCNUMBER,
       SCM.STWPROPERTYNUMBER_PK
    FROM SAPTRAN.MO_DISCHARGE_POINT MDP,
         SAP_DEL_PREM PRM,
         SAP_DEL_SCM SCM
    WHERE SCM.STWPROPERTYNUMBER_PK = PRM.STWPROPERTYNUMBER
    AND SCM.SERVICECOMPONENTTYPE = 'TE'  --SI012
    AND SCM.SPID_PK = MDP.SPID_PK;
...
--   l_no_row_dropped_cb := 0;
    ...
    LOOP
      ...
      l_err.TXT_KEY := t_tab(i).LEGACYRECNUM; 
        ...
        BEGIN
          ...
          l_progress := 'insert row into SAP_DEL_SCMTE ';

          INSERT INTO SAP_DEL_SCMTE VALUES t_tab(i);
  ...
  P_MIG_BATCH.FN_RECONLOG(no_batch, l_job.NO_INSTANCE, 'CP75', 3750, l_no_row_read,    'SCM TE (Service Components) read in from Transform tables');
  P_MIG_BATCH.FN_RECONLOG(no_batch, l_job.NO_INSTANCE, 'CP75', 3760, l_no_row_dropped, 'SCM TE (Service Components) dropped during extract from Transform tables');
  P_MIG_BATCH.FN_RECONLOG(no_batch, l_job.NO_INSTANCE, 'CP75', 3770, l_no_row_insert, 'SCM TE (Service Components) inserted to SAP_DEL_SCMTE');  
  ...
  P_MIG_BATCH.FN_RECONLOG(no_batch, l_job.NO_INSTANCE, 'CP75', 3775, l_keys_written, 'SCM TE (Service Components) distinct legacy keys inserted to SAP_DEL_SCMTE');  
  ...
  P_MIG_BATCH.FN_RECONLOG(no_batch, l_job.NO_INSTANCE, 'CP75', 3780, l_keys_written, 'SCM TE (Service Components) distinct legacy keys inserted into SAP_DEL_OUTPUT');

  SELECT COUNT(*) 
  INTO l_no_row_insert
  FROM SAP_DEL_OUTPUT;
  
  P_MIG_BATCH.FN_RECONLOG(no_batch, l_job.NO_INSTANCE, 'CP75', 3785, l_no_row_insert, 'SCM TE (Service Components) output rows inserted into SAP_DEL_OUTPUT');

    -- NOW WRITE THE FILES
  l_timestamp := TO_CHAR(SYSDATE,'YYMMDDHH24MI');
  l_filename := 'SAP_DEL_SCMTE_' || l_timestamp || '.dat';
  l_filehandle := UTL_FILE.FOPEN(l_filepath, l_filename, 'A');
  
  l_sql := 'SELECT * FROM SAP_DEL_OUTPUT WHERE KEY_COL LIKE ''TE\_%'' ESCAPE ''\'' ORDER BY KEY_COL,SECTION_ID';
...
--  l_no_row_written := l_no_row_written + l_rows_written; 
  ...
  P_MIG_BATCH.FN_RECONLOG(no_batch, l_job.NO_INSTANCE, 'CP75', 3790, l_no_row_written, 'SCM TE (Service Components) output records written to file'); 
  P_MIG_BATCH.FN_RECONLOG(no_batch, l_job.NO_INSTANCE, 'CP75', 3795, l_keys_written, 'SCM TE (Service Components) distinct legacy keys written to file'); 

  -- archive the latest batch
  P_SAP_DEL_UTIL_ARCHIVE_TABLE(p_tablename => 'SAP_DEL_SCMTE',
                           p_batch_no => no_batch,
