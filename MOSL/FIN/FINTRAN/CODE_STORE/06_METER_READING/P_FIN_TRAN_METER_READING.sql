create or replace
PROCEDURE  P_FIN_TRAN_METER_READING (no_batch     IN MIG_BATCHSTATUS.NO_BATCH%TYPE,
...
-----------------------------------------------------------------------------------------
--prerequsite
  c_module_name                 CONSTANT VARCHAR2(30) := 'P_FIN_TRAN_METER_READING';  -- modify
...
COMMIT;
  ...
  LOOP
      ...
      LOOP
        ...
        L_ERR.TXT_KEY := t_met(I).SAPEQUIPMENT; -- modify
        L_MO := null;
        L_REC_EXC := false;
        L_REC_WAR := FALSE;
        l_rec_written := true;
