...
-- ---------   ---------------     -------        --------------------------------------
-- V 1.14      03/10/2016          D.Cheung       Comment out P_DEL_UTIL_BATCH_STATS and move into separate call after batch.
-- V 1.13      15/09/2016          K.Burton       Added P_SAP_DEL_UTIL_MPDS
-- V 1.12      15/09/2016          D.Cheung       Add P_SAP_DEL_UTIL_BATCH_STATS
-- v 1.11      09/09/2016          D.Cheung       Performance - Comment out gather stats as no longer required
-- v 1.10      02/08/2016          L.Smith        Performance issues
...
BEGIN
   ...
   -- Call extracts
   -- 1(a). Point of Delivery (POD)
   g_proc_name := 'P_SAP_DEL_POD';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_POD(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_POD');
   END IF;
   
    -- 1(b). Point of Delivery (PODMO)
   g_proc_name := 'P_SAP_DEL_POD_MO';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_POD_MO(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_PODMO');
   END IF;
   
    -- 1(c). Point of Delivery (PODSRV)
   g_proc_name := 'P_SAP_DEL_POD_SRV';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_POD_SRV(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_PODSRV');
   END IF;   
   
   -- 2(a). Connection Object (COB)
   g_proc_name := 'P_SAP_DEL_COB';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_COB(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_COB');
   END IF;
   
      -- 2(b). Connection Object (COBMO)
   g_proc_name := 'P_SAP_DEL_COB_MO';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_COB_MO(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_COBMO');
   END IF;
   
   -- 2(c). Connection Object (PREM)
   g_proc_name := 'P_SAP_DEL_PREM';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_PREM(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_PREM');
   END IF;   
   
    -- 3(a). Device (DEV)
   g_proc_name := 'P_SAP_DEL_DEV';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_DEV(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_DEV');
   END IF;   

    -- 3(b). Device (DEVMO)
   g_proc_name := 'P_SAP_DEL_DEVMO';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_DEVMO(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_DEVMO');
   END IF; 

    -- 3(c). Device (DEVCHAR)
   g_proc_name := 'P_SAP_DEL_DEVCHAR';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_DEVCHAR(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_DEVCHAR');
   END IF;    

    -- 3(d). Device (DVLCRT)
   g_proc_name := 'P_SAP_DEL_DVLCRT';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_DVLCRT(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_DVLCRT');
   END IF; 

    -- 3(e). Device (DVLUPDATE)
   g_proc_name := 'P_SAP_DEL_DVLUPDATE';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_DVLUPDATE(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_DVLUPDATE');
   END IF; 
   
    -- 4(a). Meter (READ)
   g_proc_name := 'P_SAP_DEL_METER_READ';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_METER_READ(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_METER_READ');
   END IF; 

    -- 5(a). Service Component (SCM)
   g_proc_name := 'P_SAP_DEL_SCM';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_SCM(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_SCM');
   END IF;  

    -- 5(b). Service Component (SCMMO)
   g_proc_name := 'P_SAP_DEL_SCM_MO';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_SCM_MO(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_SCMMO');
   END IF;  

    -- 5(c). Service Component (SCMTE)
   g_proc_name := 'P_SAP_DEL_SCM_TE';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_SCM_TE(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_SCMTE');
   END IF;  
   
    -- 5(c). Service Component (SCMTEME)
   g_proc_name := 'P_SAP_DEL_SCM_TE_MO';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_SCM_TE_MO(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_SCMTEMO');
   END IF;  
   
   -- 4(b). Meter (INSTALL)
   g_proc_name := 'P_SAP_DEL_METER_INSTALL';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_METER_INSTALL(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_METER_INSTALL');
   END IF;    

   -- 6(a). Register Relationship (REG)
   g_proc_name := 'P_SAP_DEL_REG';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_REG(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_REG');
   END IF;       
      
   -- 7(a). Business Partners (BP)
   g_proc_name := 'P_SAP_DEL_BP';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling SAP Extract procedure - ' || g_proc_name ;
   P_SAP_DEL_BP(g_no_batch, g_no_job, return_code);
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPDEL', 'SAP_DEL_BP');
   END IF; 
   
   -- 8. Generate reference data
   P_SAP_DEL_UTIL_REF_DATA();
   
--   -- 9. Generate batch statistics data
--   P_SAP_DEL_UTIL_BATCH_STATS();
   
   -- 10. Generate MPDS (settlement) data
   P_SAP_DEL_UTIL_MPDS();   
