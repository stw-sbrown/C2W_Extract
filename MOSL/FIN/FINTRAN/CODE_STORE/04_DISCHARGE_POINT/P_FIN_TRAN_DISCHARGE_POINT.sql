CREATE OR REPLACE PROCEDURE P_FIN_TRAN_DISCHARGE_POINT(no_batch          IN MIG_BATCHSTATUS.NO_BATCH%TYPE,
...
--
-- FILENAME       : P_FIN_TRAN_DISCHARGE_POINT.sql
--
-- Subversion $Revision: 5313 $
--
-- CREATED        : 04/08/2016
--
-- DESCRIPTION    : Procedure to populate MO_DISCHARGE_POINT from SAP and OWC supplied data 
--               - OWC_DISCHARGE_POINT and SAP_DISCHARGE_POINT.
--
--
-- NOTES  :
...
-----------------------------------------------------------------------------------------
  ...
  c_module_name                 CONSTANT VARCHAR2(30) := 'P_FIN_TRAN_DISCHARGE_POINT';
...
CURSOR cur_prop (p_property_start   MO_ELIGIBLE_PREMISES.STWPROPERTYNUMBER_PK%TYPE,
    ...
    WHERE   pr.STWPROPERTYNUMBER_PK BETWEEN p_property_start AND p_property_end  --AND pr.STWPROPERTYNUMBER_PK = 958002009
...
WHEN OTHERS THEN     
     ...
     P_MIG_BATCH.FN_UPDATEJOB(no_batch, l_job.NO_INSTANCE, l_job.IND_STATUS);
     return_code := -1;
END P_FIN_TRAN_DISCHARGE_POINT;
/
show error;
