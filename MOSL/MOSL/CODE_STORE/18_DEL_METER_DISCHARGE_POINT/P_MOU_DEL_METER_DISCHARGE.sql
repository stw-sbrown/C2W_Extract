...
--
--            UTL_FILE.PUT(fHandle,t_meter_dp(i).MANUFACTURERSERIALNUM_PK || '|');
--            UTL_FILE.PUT(fHandle,t_meter_dp(i).MANUFACTURER_PK || '|');
--            UTL_FILE.PUT(fHandle,t_meter_dp(i).DPID_PK || '|');
--            UTL_FILE.PUT_LINE(fHandle,t_meter_dp(i).PERCENTAGEDISCHARGE);
--
--          EXCEPTION
--            WHEN OTHERS THEN
--                 l_rec_written := FALSE;
    ...
    CASE w.WHOLESALER_ID 
      WHEN 'ANGLIAN-W' THEN
        ...
        IF w.RUN_FLAG = 0 THEN
          SELECT COUNT(*) INTO l_count FROM DEL_METER_DISCHARGE_ANW_V;
        END IF;
      WHEN 'DWRCYMRU-W' THEN
        l_sql := 'SELECT * FROM DEL_METER_DISCHARGE_WEL_V';
        l_filename := 'METER_DISCHARGE_' || w.WHOLESALER_ID || '_' || TO_CHAR(SYSDATE,'YYMMDDHH24MI') || '.dat';
        IF w.RUN_FLAG = 0 THEN
          SELECT COUNT(*) INTO l_count FROM DEL_METER_DISCHARGE_WEL_V;
        END IF;
      WHEN 'SEVERN-W' THEN
        l_sql := 'SELECT * FROM DEL_METER_DISCHARGE_STW_V';
        l_filename := 'METER_DISCHARGE_' || w.WHOLESALER_ID || '_' || TO_CHAR(SYSDATE,'YYMMDDHH24MI') || '.dat';
        IF w.RUN_FLAG = 0 THEN
          SELECT COUNT(*) INTO l_count FROM DEL_METER_DISCHARGE_STW_V;
        END IF;
      WHEN 'THAMES-W' THEN
        l_sql := 'SELECT * FROM DEL_METER_DISCHARGE_THW_V';
        l_filename := 'METER_DISCHARGE_' || w.WHOLESALER_ID || '_' || TO_CHAR(SYSDATE,'YYMMDDHH24MI') || '.dat';
        IF w.RUN_FLAG = 0 THEN
          SELECT COUNT(*) INTO l_count FROM DEL_METER_DISCHARGE_THW_V;
        END IF;
      WHEN 'WESSEX-W' THEN
        l_sql := 'SELECT * FROM DEL_METER_DISCHARGE_WEW_V';
        l_filename := 'METER_DISCHARGE_' || w.WHOLESALER_ID || '_' || TO_CHAR(SYSDATE,'YYMMDDHH24MI') || '.dat';
        IF w.RUN_FLAG = 0 THEN
          SELECT COUNT(*) INTO l_count FROM DEL_METER_DISCHARGE_WEW_V;
        END IF;
      WHEN 'YORKSHIRE-W' THEN
        l_sql := 'SELECT * FROM DEL_METER_DISCHARGE_YOW_V';
        l_filename := 'METER_DISCHARGE_' || w.WHOLESALER_ID || '_' || TO_CHAR(SYSDATE,'YYMMDDHH24MI') || '.dat';
        IF w.RUN_FLAG = 0 THEN
          SELECT COUNT(*) INTO l_count FROM DEL_METER_DISCHARGE_YOW_V;
        END IF;
      WHEN 'UNITED-W' THEN
        l_sql := 'SELECT * FROM DEL_METER_DISCHARGE_UUW_V';
        l_filename := 'METER_DISCHARGE_' || w.WHOLESALER_ID || '_' || TO_CHAR(SYSDATE,'YYMMDDHH24MI') || '.dat';
        IF w.RUN_FLAG = 0 THEN
          SELECT COUNT(*) INTO l_count FROM DEL_METER_DISCHARGE_UUW_V;
        END IF;
      WHEN 'SOUTHSTAFF-W' THEN
        l_sql := 'SELECT * FROM DEL_METER_DISCHARGE_SSW_V';
        l_filename := 'METER_DISCHARGE_' || w.WHOLESALER_ID || '_' || TO_CHAR(SYSDATE,'YYMMDDHH24MI') || '.dat';
        IF w.RUN_FLAG = 0 THEN
          SELECT COUNT(*) INTO l_count FROM DEL_METER_DISCHARGE_SSW_V;
        END IF;
    END CASE;
