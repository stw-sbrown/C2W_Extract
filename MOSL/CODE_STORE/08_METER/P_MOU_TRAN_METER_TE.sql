...
-- V 3.13      05/10/2016          D.Cheung               I-366 - Set period data set to look at period 16 only as period 17 missing some meters.
-- V 3.14      30/11/2016          D.Cheung               MOSL clarification - Set Sewchargeablemetersize to NULL for PRIVATEWATER
-----------------------------------------------------------------------------------------
--prerequsite
...
--           (SELECT NO_IWCS, MAX(PERIOD) MAXPERIOD, MAX(LENGTH(ROUND(ABS(START_READ)))) LEN_START, MAX(LENGTH(ROUND(ABS(END_READ)))) LEN_END FROM BT_TE_WORKING GROUP BY NO_IWCS ) BTWM,   --3.13
           (SELECT NO_IWCS, 16 AS MAXPERIOD, MAX(LENGTH(ROUND(ABS(START_READ)))) LEN_START, MAX(LENGTH(ROUND(ABS(END_READ)))) LEN_END FROM BT_TE_WORKING GROUP BY NO_IWCS ) BTWM,     --3.13
...
--*** v3.12 - check for service component
                ...
                ELSIF T_MET(I).METERTREATMENT = 'PRIVATEWATER' THEN 
                   T_MET(I).SEWCHARGEABLEMETERSIZE := NULL;
                END IF;
