create or replace
PROCEDURE P_MOU_TRAN_KEY_GEN_SAP(no_batch    IN MIG_BATCHSTATUS.NO_BATCH%TYPE,
...
--
-- FILENAME       : P_MOU_TRAN_KEY_GEN_SAP.sql
--
-- Subversion $Revision: 6391 $
--
-- CREATED        : 24/02/2016
--
...
-- ---------   ----------  --------   --------------------------------------------------
-- V 2.18      21/11/2016  D.Cheung   Update BT_MISS_AG_SC to pick up missing TE services due to aggregates
-- V 2.17      30/09/2016  S.Badhan   Update for new index BT_TVP054_IDX4.
-- V 2.16      27/09/2016  S.Badhan   Fix to OWC bt table
-- V 2.15      26/09/2016  S.Badhan   Performance changes - add parallel processing.
-- V 2.14      16/09/2016  S.Badhan   Amend to use SAP ready flag.
...
-----------------------------------------------------------------------------------------
  ...
  c_module_name                 CONSTANT VARCHAR2(30) := 'P_MOU_TRAN_KEY_GEN_SAP';
  l_key                         VARCHAR2(30);
...
BEGIN
  ...
  INSERT /*+ append */ INTO BT_TVP054
   ...
   FG_SAP_RDY, FG_SAP_LOADED, TS_SAP_LOADED)    
...
AND    tnhh.FG_SAP_RDY        in (SELECT PHASE
                                  FROM   MIG_PHASE_KEYGEN
                                  WHERE  FG_INLC_BATCH = 'Y'
                                  AND    FG_KEYGEN_PRC = 'N')
AND    tnhh.FG_SAP_LOADED     = 'N'
...
-- V 2.10    
  ...
  l_progress := 'INSERT BT_OWC_CUST_SWITCHED_SUPPLIER';
   ...
   ,t1.FG_MO_RDY, t1.FG_MO_LOADED, t1.TS_MO_LOADED, t1.FG_SAP_RDY, t1.FG_SAP_LOADED, t1.TS_SAP_LOADED
   ...
   AND    t1.FG_SAP_RDY    in (SELECT PHASE
                               FROM   MIG_PHASE_KEYGEN
                               WHERE  FG_INLC_BATCH = 'Y'
                               AND    FG_KEYGEN_PRC = 'N')
   AND    t1.FG_SAP_LOADED = 'N'
   join CIS.ELIGIBILITY_CONTROL_TABLE t3 on t3.CD_COMPANY_SYSTEM = t1.CD_COMPANY_SYSTEM AND  t3.NO_PROPERTY  = t1.NO_PROPERTY
   left join LU_SAP_FLOCA t4 on    t3.NO_PROPERTY       = t4.STWPROPERTYNUMBER_PK
   ) x
   where Record_Nr = 1;
   ...
   from BT_OWC_CUST_SWITCHED_SUPPLIER;
    ...
    l_progress := 'INSERT BT_TVP163';
      ...
      SELECT /*+ PARALLEL(telg,60) PARALLEL(keygen,60) PARALLEL(t202,60) */      
            ...
            keygen.FG_NMM,
            keygen.FG_SAP_RDY, 
            keygen.FG_SAP_LOADED, 
            keygen.TS_SAP_LOADED
