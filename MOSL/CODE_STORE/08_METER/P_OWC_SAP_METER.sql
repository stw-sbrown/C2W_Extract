CREATE OR REPLACE PROCEDURE P_OWC_SAP_METER(no_batch          IN MIG_BATCHSTATUS.no_batch%TYPE,
...
-- AUTHOR         : Kevin Burton
--
-- FILENAME       : P_OWC_SAP_METER.sql
--
-- Subversion $Revision: 5840 $
--
-- CREATED        : 13/10/2016
--
-- DESCRIPTION    : Procedure to populate MO_METER, SAP_ADDRESS and SAP_METER_ADDRESS from OWC supplied data 
--                  - OWC_METER.
--
--
-- NOTES  :
...
-- ---------   ----------  -------    ---------------------------------------------------
-- V 0.01      13/10/2016  K.Burton   Initial version
-----------------------------------------------------------------------------------------

  c_module_name                 CONSTANT VARCHAR2(30) := 'P_OWC_SAP_METER';
  ...
  l_adr_mt                      SAP_METER_ADDRESS%ROWTYPE := NULL;  
...
CURSOR cur_prop (p_property_start   MO_ELIGIBLE_PREMISES.STWPROPERTYNUMBER_PK%TYPE,
    ...
    SELECT  OM.MANUFACTURERSERIALNUM_PK,
            ...
            OM.RETURNTOSEWER,
            OM.WATERCHARGEMETERSIZE,
            OM.SEWCHARGEABLEMETERSIZE,
...
--              END IF;
          ...
          END IF; -- SPID is not NULL
            ...
            ELSE -- if we have a property address key from main query check that the address is the same as the meter
              ...
              IF (t_prop(i).PRIMARYADDRESSABLEOBJECT = l_adr.STREET3 AND
                  t_prop(i).SECONDADDRESABLEOBJECT = l_adr.STREET2 AND
                  t_prop(i).ADDRESSLINE01 = l_adr.STREET AND
                  t_prop(i).ADDRESSLINE02 = l_adr.STREET4 AND
                  t_prop(i).ADDRESSLINE03 = l_adr.STREET5 AND
                  t_prop(i).ADDRESSLINE04 = l_adr.DISTRICT AND
                  t_prop(i).ADDRESSLINE05 = l_adr.CITY AND
            ...
            IF l_add_new_address THEN
              ...
              SELECT MAX(ADDRESS_PK)+1 INTO l_adr.ADDRESS_PK FROM SAP_ADDRESS;
              ...
              l_adr.STREET2 := t_prop(i).SECONDADDRESABLEOBJECT;
              l_adr.STREET3 := t_prop(i).PRIMARYADDRESSABLEOBJECT; 
              l_adr.HOUSENUMBER := NULL;  -- house number (if there is one) is included in STREET
              l_adr.STREET := t_prop(i).ADDRESSLINE01;
              l_adr.STREET4 := t_prop(i).ADDRESSLINE02;
              l_adr.STREET5 := t_prop(i).ADDRESSLINE03;
              l_adr.DISTRICT := t_prop(i).ADDRESSLINE04;
              l_adr.CITY := t_prop(i).ADDRESSLINE05;
              ...
              BEGIN 
                ...
                (l_adr.ADDRESS_PK,l_adr.UPRN,l_adr.UPRNREASONCODE,l_adr.PAFADDRESSKEY,
                 ...
                 l_adr.LOCATIONFREETEXTDESCRIPTOR,NULL,NULL,owc.OWC);   
             ...
             l_progress := 'INSERT SAP_METER_ADDRESS';     
             BEGIN 
                INSERT INTO SAP_METER_ADDRESS 
