create or replace
PROCEDURE P_FIN_TRAN_METER_SPID_ASSOC (no_batch     IN MIG_BATCHSTATUS.NO_BATCH%TYPE,
...
-- AUTHOR         : Ola Badmus
--
-- FILENAME       : P_FIN_TRAN_METER_SPID_ASSOC.sql
--
...
-----------------------------------------------------------------------------------------
--prerequsite
  c_module_name                 CONSTANT VARCHAR2(30) := 'P_FIN_TRAN_METER_SPID_ASSOC'; 
  ...
  L_MO                          MO_METER_SPID_ASSOC%ROWTYPE; --modify
...
CURSOR cur_met (P_METERREF_START   MO_METER_READING.METERREF%type,
    ...
    FROM   RECEPTION.SAP_METER_SUPPLY_POINT t1
           INNER JOIN RECEPTION.SAP_METER t2
...
WHEN OTHERS THEN
      ...
      commit;
      RETURN_CODE := -1;
END P_FIN_TRAN_METER_SPID_ASSOC;
/
show error;
