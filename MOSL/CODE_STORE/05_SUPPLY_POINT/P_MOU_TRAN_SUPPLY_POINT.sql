create or replace
PROCEDURE P_MOU_TRAN_SUPPLY_POINT(no_batch          IN MIG_BATCHSTATUS.NO_BATCH%TYPE,
...
-- ---------   ----------  -------    ---------------------------------------------------
-- V 1.12      30/11/2016  K.Burton   Add check for ANGLIAN-W SPID coming from TARGET - set PAIRINGREFREASONCODE = NULL and OTHERWHOLESALERID = 'ANGLIAN-W'
-- V 1.11      23/11/2016  S.Badhan   Add union to BT_MISS_AG_SC to get dropped supply on aggregates
...
-- V 1.06      29/06/2016  O.Badmus   CR_022 Flag Retailer of SPID for OWC. This is for specific account which have already switched retailers before 2017
-- V 1.05      23/06/2016  D.Cheung   I-255 - Change for Welsh SPIDs  (i.e. wholesaler = DWRCYMRU-W) need to set Pairing Reference Reason Code = ?NOTELIGIBLE?
-- V 1.04      16/06/2016  S.Badhan   SI-001. Set SPIDSTATUS to TRADABLE.
...
-----------------------------------------------------------------------------------------
  ...
  w_swi                         LU_SPID_OWC_RETAILER%ROWTYPE;
...
SELECT  DISTINCT  -- V 1.08
           ...
           t054.DT_END,
           t054.CORESPID,
           cast(t054.SAP_FLOC as varchar2(30)) SAP_FLOC,
           tcat.SUPPLY_POINT_CODE,
           t056.DT_STATUS,
           t054.ST_SERV_PROV,
           t056.DT_START AS T056_DT_START
...
SELECT DISTINCT
           ...
           t056.ST_SERV_PROV,
           t056.DT_START AS T056_DT_START
      FROM   BT_MISS_AG_SC       t054,
             LU_SERVICE_CATEGORY tcat,
             CIS.TVP056SERVPROV  t056,
             (select distinct CORESPID, SAP_FLOC, NO_PROPERTY_MASTER
              from   bt_tvp054 where NO_PROPERTY_MASTER is not null) mas
      WHERE  t054.NO_PROPERTY BETWEEN p_no_property_start AND p_no_property_end
      AND    tcat.TARGET_SERV_PROV_CODE = t054.CD_SERVICE_PROV
      AND    t056.CD_COMPANY_SYSTEM = t054.CD_COMPANY_SYSTEM
      AND    t056.NO_PROPERTY       = t054.NO_PROPERTY
      AND    t056.NO_SERV_PROV      = t054.NO_SERV_PROV
      AND    t054.NO_PROPERTY_MASTER  = mas.NO_PROPERTY_MASTER
ORDER BY NO_PROPERTY,
         SUPPLY_POINT_CODE,
         T056_DT_START,
         DT_END DESC NULLS FIRST;
    ...
    LOOP
      ...
      IF l_prev_prp || l_prev_spt || l_prev_cus <> t_prop(i).NO_PROPERTY || t_prop(i).SUPPLY_POINT_CODE || t_prop(i).NO_LEGAL_ENTITY THEN
            ...
            END;
             ...
             BEGIN   -- V 1.06
               SELECT WCRETAILER
               INTO   w_swi.WCRETAILER
               FROM   LU_SPID_OWC_RETAILER
               WHERE  STWPROPERTYNUMBER_PK  = t_prop(i).NO_PROPERTY
               and    STWSERVICETYPE = T_PROP(I).SUPPLY_POINT_CODE;
            EXCEPTION
           WHEN NO_DATA_FOUND THEN
                w_swi.WCRETAILER :=  l_brd.RETAILERID_PK;
            END;

            --l_sp.RETAILERID_PK := l_brd.RETAILERID_PK;
            l_sp.RETAILERID_PK := w_swi.WCRETAILER;  -- V 1.06
...
-- v1.07 - Change for Welsh SPIDs
                IF (l_sp.WHOLESALERID_PK  = 'DWRCYMRU-W') THEN
                    l_sp.PAIRINGREFREASONCODE := 'NOTELIGIBLE';
                END IF;
-- v1.07
...
--*** v1.05 - Change for Welsh SPIDs
--                IF (l_brd.WHOLESALERID_PK = 'DWRCYMRU-W') THEN
--                    l_sp.PAIRINGREFREASONCODE := 'NOTELIGIBLE';
--                END IF;
--*** v1.05
             ELSE
...
-- V 1.12
            ...
            AND ID_OWC = 'ANGLIAN-W';
            
            IF l_count > 0 THEN
              l_sp.OTHERWHOLESALERID := 'ANGLIAN-W';
              l_sp.PAIRINGREFREASONCODE := NULL;
            END IF;
-- V 1.12
