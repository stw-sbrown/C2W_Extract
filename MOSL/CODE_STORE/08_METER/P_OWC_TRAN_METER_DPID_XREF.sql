create or replace
PROCEDURE P_OWC_TRAN_METER_DPID_XREF (no_batch     IN MIG_BATCHSTATUS.NO_BATCH%TYPE,
...
--                  Will read from receptiond OWC_METER_READING tables
-- NOTES  :
...
-----------------------------------------------------------------------------------------
  ...
  c_module_name                 CONSTANT VARCHAR2(30) := 'P_OWC_TRAN_METER_DPID_XREF';  
...
CURSOR cur_met (P_METERREF_START   MO_METER_READING.METERREF%type,
    ...
    is
     ...
     FROM (SELECT  NULL DPEFFECTFROMDATE,
           ...
           FROM    RECEPTION.OWC_METER_DISCHARGE_POINT dp
...
when OTHERS then
      ...
      commit;
      RETURN_CODE := -1;
END P_OWC_TRAN_METER_DPID_XREF;
/
show error;
