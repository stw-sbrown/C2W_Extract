...
--          IF (l_master_dropped = true AND t_met(i).MAIN_SPID_PK IS NULL) THEN
--              P_MIG_BATCH.FN_ERRORLOG(NO_BATCH, L_JOB.NO_INSTANCE, 'X', SUBSTR('NON-NETWORK-SUB MAIN CANNOT BE NON_MARKET WHERE MASTER DROPPED',1,100),  L_ERR.TXT_KEY, SUBSTR(l_err.TXT_DATA || ',' || L_PROGRESS,1,100));
          IF (l_master_dropped = true OR l_master_written = false) THEN
              ...
              L_NO_ROW_EXP := L_NO_ROW_EXP + 1;
              L_REC_EXC := true;
          ELSIF (t_met(i).MAIN_NMM = 'N' AND t_met(i).MAIN_SPID_PK IS NULL) THEN
