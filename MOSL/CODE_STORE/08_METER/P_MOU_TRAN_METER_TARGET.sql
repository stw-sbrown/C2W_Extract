...
-- ---------   ---------------     -------        ------  ----------------------------------
-- v 9.27      14/11/2016          D.Cheung               I-376 - Add logic to dataloggers to match on propertynumber only if cannot match on property and Serialnum
-- v 9.26      01/11/2016          D.Cheung               REOPEN - I-352 - Add workaround for Aggregate nonmarketflag issue
-- v 9.25      15/09/2016          D.Cheung               I-360 - default NULL RTS value ausing problems when trying to calculate TE SMA values - change default to 100
...
--prerequsite
  ...
  l_property_agg_ect            NUMBER;   --v9.26
  l_datalogger_found            BOOLEAN;  --v9.27
...
CURSOR CUR_MET (P_NO_EQUIPMENT_START   CIS.TVP043METERREG.NO_EQUIPMENT%type,
    ...
    IS
      SELECT /*+ FULL(T163) FULL(TV163) FULL(T043) FULL(T225) FULL(TV163_S) FULL(TV054) FULL(TV163_RTS) PARALLEL(T703,12) PARALLEL(TV163,12)  PARALLEL(T034,12) PARALLEL(T036,12) */
...
--          ,CASE WHEN NVL(TRIM(TV163_I.FG_NMM),'Y') = 'Y' THEN NULL ELSE TRIM(LSR.SPID_PK) END SPID_PK -- V 3.02   v9.18
          ,TRIM(TV054.CORESPID) AS CORESPID   --v9.26
          ,TRIM(LSR.SPID_PK) AS SPID_PK       --v9.26
...
--          , TV163.NO_PROPERTY_MASTER -- v9.07
          ...
          ,MAX(NVL(TV163_RTS.PC_USAGE_SPLIT,1) * 100) AS RETURNTOSEWER   --v9.22  --v9.25
...
-- *** v9.26 - double-check nonmarketflag settings
            IF (t_met(i).NONMARKETMETERFLAG = 1) THEN
                IF (t_met(i).NO_PROPERTY_MASTER IS NULL) THEN
                    t_met(i).CORESPID := NULL;
                    t_met(i).SPID_PK := NULL;
                ELSE
                    BEGIN
                        --CHECK IF AGGREGATED INSTALLPROPERTY IS IN ECT
                        SELECT COUNT(NO_PROPERTY)
                        INTO l_property_agg_ect
                        FROM CIS.ELIGIBILITY_CONTROL_TABLE
                        WHERE NO_PROPERTY = t_met(i).NO_PROPERTY
                        AND FG_NMM = 'N';
                    
                        IF (l_property_agg_ect = 0) THEN
                            t_met(i).CORESPID := NULL;
                            t_met(i).SPID_PK := NULL;
                        ELSE
                            t_met(i).NONMARKETMETERFLAG := 0;
                        END IF;
                    END;
                END IF;
            END IF;
...
--*** I-193
            ...
            L_PROGRESS := 'GET LU_DATALOGGERS BY PROPERTY AND SERIALNUM';
            BEGIN
                ...
                l_datalogger_found := TRUE;
            EXCEPTION
                when NO_DATA_FOUND then
                    l_hlt.DATALOGGERNONWHOLESALER := 0;
                    l_hlt.DATALOGGERWHOLESALER := 0;
                    l_datalogger_found := FALSE;
            END;
            
-- v9.27 (I-376)
            IF (l_datalogger_found <> TRUE) THEN
                L_PROGRESS := 'GET LU_DATALOGGERS BY PROPERTY ONLY';
                BEGIN
                    SELECT DISTINCT NVL(DATALOGGERNONWHOLESALER,0), NVL(DATALOGGERWHOLESALER,0)
                    INTO   l_hlt.DATALOGGERNONWHOLESALER, l_hlt.DATALOGGERWHOLESALER
                    FROM   LU_DATALOGGERS
