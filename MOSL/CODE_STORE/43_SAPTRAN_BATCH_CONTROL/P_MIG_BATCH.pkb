...
-- ---------   ---------------     -------        --------------------------------------
-- V 1.20      01/12/2016          D.Cheung       Comment out OWC package from batch for SAP PHASE 2
-- V 1.19      03/11/2016          S.Badhan       Add P_MOU_TRAN_METER_RTS_MDVOL and P_MOU_TRAN_METER_NETWORK_TE
-- V 1.18      12/10/2016          K.Burton       Added call to OWC batch
-- V 1.17      16/09/2016          S.Badhan       Now use SAP specifc version of key gen.
-- V 1.16      09/09/2016          D.Cheung       I-354 - Not picking up meter spid associations for TE due to batch ordering
-- V 1.15      09/09/2016          D.Cheung       Perforamnce change - comment out gather stats as no longer needed
-- V 1.14      01/08/2016          L.Smith        Performance issues due to bad stats.
-- V 1.13      19/07/2016          S.Badhan       I-260. Add proc P_MOU_TRAN_SC_AS.
-- V 1.12      27/05/2016          M.Marron       Added to SAPTRAN and added p_SAP_TRAN_Address
...
-- 1a. Key Gen
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_TVP054');
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_TVP163');
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_MISS_AG_SC');
   END IF;
...
-- 1c. Meter Rollover (Initial)
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_CLOCKOVER');
   END IF;
...
-- 1d. Service Component  (pre tables)
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SP_TARIFF');
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SP_TARIFF_ALG');
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SP_TARIFF_ALGITEM');
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SP_TARIFF_SPLIT');
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SP_TARIFF_REFTAB');
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SP_TARIFF_EXTREF');
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SPR_TARIFF');
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SPR_TARIFF_ALGITEM');
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SPR_TARIFF_EXTREF');
   END IF;
...
-- 1d. Service Component  (MPW)
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SC_MPW');
   END IF;
...
-- 1d. Service Component  (UW)
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SC_UW');
   END IF;
...
-- 1e. Service Component  (AS)
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--     DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_SC_AS');
   END IF;
...
-- 1e. TE Working
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_TE_WORKING');
   END IF;
...
-- 1f. TE Summary
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_TE_SUMMARY');
   END IF;
...
-- 5a. Meter (TARGET)
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_METER_READ_FREQ');
   END IF;
...
-- 5b. Meter to Supply Point Association
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_METER_NETWORK');
   END IF;
...
-- 8. Address (All)
   ...
   g_proc_name := 'P_SAP_TRAN_ADDRESS';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS;
   return_code := 0;
   l_progress := 'Calling MO Extract procedure - ' || g_proc_name ;
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_INSTALL_ADDRESS');
   END IF;
...
-- 9. Bad data reporting
   ...
   IF return_code = -1 THEN
      ...
      RETURN;
--   ELSE
--      DBMS_STATS.gather_table_stats('SAPTRAN', 'BT_BAD_DATA');
   END IF;  

--**** V1.20 - NOT NEEDED FOR SAP PHASE 2 *****   
   -- Call OWC batch job

--   return_code := 0;     
--   P_MIG_BATCH_SAP_OWC.P_STARTBATCH(g_no_batch, return_code);
--
--   IF return_code = -1 THEN
--      FN_UPDATEBATCH('ERR');
--      DBMS_OUTPUT.PUT_LINE(to_char(SYSDATE,'HH24:MI:SS') || ' -  MIGRATION_BATCH Ended With errors  !!!!!');
