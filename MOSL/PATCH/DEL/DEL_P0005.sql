...
-- V0.08          14/10/2016      K.Burton        Added CROSSBORDER to SEWERAGE and PRIVATEWATER processing
------------------------------------------------------------------------------------------------------------
...
BEGIN
  ...
  ELSIF l_meter_treatment = 'NONPOTABLE' THEN
    ...
    ELSE
      RAISE_APPLICATION_ERROR( -20001, 'SERVICECATEGORY is incompatible with METERTREATMENT ' || l_meter_treatment);
    END IF;
  ELSIF l_meter_treatment IN ('PRIVATEWATER','SEWERAGE','CROSSBORDER') THEN
