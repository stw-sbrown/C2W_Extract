...
--
-- FILENAME       : P_MIG_BATCH_OWC.pkb
--
-- Subversion $Revision: 6115 $
--
-- CREATED        : 22/09/2016
--
...
-- ---------   ---------------     -------        --------------------------------------
-- V 0.05      07/11/2016          S.Badhan       Remove End of batch update as done in the main calling proc
-- V 0.04      28/10/2016          S.Badhan       Include proc P_OWC_TRAN_TE_METER_DPID_XREF for Phase 3 processing
-- V 0.03      11/10/2016          S.Badhan       Remove procs not required for sewage processing
-- V 0.02      10/10/2016          S.Badhan       Send return code from P_STARTBATCH.
-- V 0.01      22/09/2016          S.Badhan       Initial Draft
----------------------------------------------------------------------------------------
...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LINE(to_char(SYSDATE,'HH24:MI:SS') || ' -  OWC MIGRATION_BATCH Started ');
   ...
   -- 1. Supply Point
   g_proc_name := 'P_OWC_TRAN_SUPPLY_POINT';
   ...
   P_OWC_TRAN_SUPPLY_POINT(g_no_batch, g_no_job, return_code);  
   
   IF return_code = -1 THEN
      FN_UPDATEBATCH('ERR');
      DBMS_OUTPUT.PUT_LINE(to_char(SYSDATE,'HH24:MI:SS') || ' -  OWC MIGRATION_BATCH Ended With errors  !!!!!');
      lreturn_code := -1;
      RETURN;
   END IF;
 
   -- 2. Service Component
   g_proc_name := 'P_OWC_TRAN_SERVICE_COMPONENT';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS; 
   return_code := 0;
   l_progress := 'Calling MO Extract procedure - ' || g_proc_name ;
   DBMS_OUTPUT.PUT_LINE(l_progress);
   P_OWC_TRAN_SERVICE_COMPONENT(g_no_batch, g_no_job, return_code);  
   ...
   -- 4. Meter   
   g_proc_name := 'P_OWC_TRAN_METER';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS; 
   return_code := 0;
   l_progress := 'Calling MO Extract procedure - ' || g_proc_name ;
   DBMS_OUTPUT.PUT_LINE(l_progress);
   P_OWC_TRAN_METER(g_no_batch, g_no_job, return_code);  
   ...
   -- 5. Meter Reading
   g_proc_name := 'P_OWC_TRAN_METER_READING';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS; 
   return_code := 0;
   l_progress := 'Calling MO Extract procedure - ' || g_proc_name ;
   DBMS_OUTPUT.PUT_LINE(l_progress);
   P_OWC_TRAN_METER_READING(g_no_batch, g_no_job, return_code);  
   ...
   -- 6. Meter SPID Association 
   g_proc_name := 'P_OWC_TRAN_METER_SPID_ASSOC';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS; 
   return_code := 0;
   l_progress := 'Calling MO Extract procedure - ' || g_proc_name ;
   DBMS_OUTPUT.PUT_LINE(l_progress);
   P_OWC_TRAN_METER_SPID_ASSOC(g_no_batch, g_no_job, return_code);  
   ...
   --7. Meter DPID TE cross reference
   g_proc_name := 'P_OWC_TRAN_TE_METER_DPID_XREF';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS; 
   return_code := 0;
   l_progress := 'Calling MO Extract procedure - ' || g_proc_name ;
   DBMS_OUTPUT.PUT_LINE(l_progress);
   P_OWC_TRAN_TE_METER_DPID_XREF(g_no_batch, g_no_job, return_code);  
  ...
  DBMS_OUTPUT.PUT_LINE(to_char(sysdate,'HH24:MI:SS') || ' -  OWC MIGRATION_BATCH Ended ');
...
WHEN OTHERS THEN     
     ...
     ind_status := 'ERR';
     P_MIG_BATCH_OWC.FN_UPDATEJOB(no_batch, no_instance, ind_status);   
END FN_STARTJOB;
