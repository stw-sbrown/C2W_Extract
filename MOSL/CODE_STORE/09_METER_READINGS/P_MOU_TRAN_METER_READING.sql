...
-- CR01 - CHANGE CURSOR TO ONLY GET METERS WRITTEN TO MO_METER
      SELECT DISTINCT * FROM (
          ...
          DISTINCT
                ...
                , MO.INSTALLEDPROPERTYNUMBER    --*** v7.02
                , MO.MANUFCODE                  --*** v7.02
                , MO.SPID_PK                    --V8.01
          ...
          JOIN CIS.TVP063EQUIPMENT T063 ON (T063.NO_EQUIPMENT = MO.METERREF AND TRIM(T063.CD_COMPANY_SYSTEM) = 'STW1')
...
--              AND MO.NONMARKETMETERFLAG = 0   --v9.07
          ...
          UNION
          SELECT * FROM (
              SELECT /*+ PARALLEL(MO,12) PARALLEL(T195,12) PARALLEL(MSP,12) */
