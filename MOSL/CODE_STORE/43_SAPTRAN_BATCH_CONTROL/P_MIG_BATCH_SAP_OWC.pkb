create or replace
PACKAGE BODY P_MIG_BATCH_SAP_OWC
IS
 ------------------------------------------------------------------------------------
...
--
-- FILENAME       : P_MIG_BATCH_SAP_OWC.pkb
--
...
BEGIN
   ...
   -- 1. Supply Point
   g_proc_name := 'P_OWC_SAP_SUPPLY_POINT';
   ...
   P_OWC_SAP_SUPPLY_POINT(g_no_batch, g_no_job, return_code);  
   ...
   -- 4. Meter   
   g_proc_name := 'P_OWC_SAP_METER';
   l_progress := 'Get job parameters ' || g_proc_name;
   FN_GETJOB_PARMS; 
   return_code := 0;
   l_progress := 'Calling MO Extract procedure - ' || g_proc_name ;
   DBMS_OUTPUT.PUT_LINE(l_progress);
   P_OWC_SAP_METER(g_no_batch, g_no_job, return_code);  
...
WHEN OTHERS THEN     
     ...
     ind_status := 'ERR';
     P_MIG_BATCH_SAP_OWC.FN_UPDATEJOB(no_batch, no_instance, ind_status);   
END FN_STARTJOB;
